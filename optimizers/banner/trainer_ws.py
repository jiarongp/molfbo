...
class DistanceEstimator(torch.nn.Module):
    def __init__(self, input_dim):
        ...
        self.layers = torch.nn.Sequential(
            spectral_norm(torch.nn.Linear(input_dim, 64)),
            torch.nn.LeakyReLU(0.2, inplace=True),
            # spectral_norm(torch.nn.Linear(64, 64)),
            # torch.nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(torch.nn.Linear(64, 64)),
            torch.nn.LeakyReLU(0.2, inplace=True),
            spectral_norm(torch.nn.Linear(64, 1)),
        )
...
def training_loop(
    ...
    de = DistanceEstimator(model.task_embedding_size)
    optimizer_de = torch.optim.Adam(de.parameters(), lr=1e-3, betas=(0.5, 0.999))
    ...
    with tqdm(range(num_epochs)) as pbar:
        for epoch in pbar:
            ...
            for _, batch_data in enumerate(train_dataloader):
                ...
                prior_samples = torch.randn(model.task_embedding.weight[1:].shape)
