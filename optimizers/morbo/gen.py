            ...
            if not any(feas):  # Ignore the objectives if all are infeasible
                ...
                if trbo_state.tr_hparams.hypervolume:
                    ...
                    if any(better_than_ref):
                        ...
                        # NOTE: replace the qEHVI with Thompson sampling
                        ...
                        with torch.no_grad():
                            ...
                            # NOTE: this doesn't work in new versions of BoTorch
                            # hvi = acqf(  # dummy input
                            #     X_cand_unnormalized[better_than_ref].unsqueeze(1)
                            # ).to(device=tkwargs["device"])
                            # NOTE: change to this
                            posterior = sampled_model.posterior(
                                X_cand_unnormalized[better_than_ref].unsqueeze(1)
                            )
                            # add sample dimension to compute hypervolume contribution
                            hvi = acqf._compute_qehvi(posterior.mean.unsqueeze(0)).to(device=tkwargs["device"])
