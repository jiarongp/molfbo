...
class MBORE_MDRE_EI_AUX_VAR_001:
    ...
    def observe_and_suggest(self, X_obs, y_obs, X_pen=None, batch_size=1, gamma=1/3, var_threshold=0.01, *args, **kwargs):
        ...
        self.kl = self.estimate_kl_divergence()
        print(f"KL divergence: {self.kl}")
        print(f"{1 - (-self.kl).exp()}")
        # dr_mdre = pred_logits[:, 0] - pred_logits[:, 1]
        if self.variance_ratio >= var_threshold:
            dr_mdre = acf_mdre[:, 0] / acf_mdre[:, 1]
        else:
            dr_mdre = acf_mdre[:, 2] / acf_mdre[:, 1]
