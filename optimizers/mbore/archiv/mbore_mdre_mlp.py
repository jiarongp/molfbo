...
class MBORE_MDRE:
    ...
    def generate_mdre_dataset(self, X, y, gamma, acq_type='pi'):
    ...
    def fit_model(self, X_obs, y_obs, gamma, batch_size=256, S=100):
        self.clf = MLP(
        ...
        x, z, w, self.class_weight = self.generate_mdre_dataset(
        ...
        for epochs in range(E):
            for _, (inputs, targets, weights) in enumerate(train_dataloader):
                ...
                batch_loss = loss_fn(
                    outputs, targets, weight=weights, class_weight=self.class_weight
                )
                batch_loss.backward()
                optimizer.step()
    ...
    def observe_and_suggest(self, X_obs, y_obs, X_pen=None, batch_size=1, gamma=1/3, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            acf_mdre = torch.nn.functional.softmax(pred_logits + torch.log(self.class_weight), dim=-1)
            # dr_mdre = 1 - acf_mdre[:, 1]
            dr_mdre = acf_mdre[:, 0].log() - acf_mdre[:, 1].log() + acf_mdre[:, 2].log()
