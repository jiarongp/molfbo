...
class MBORE_MDRE_EI_AUX:
    ...
    def generate_mdre_dataset(self, X, y, gamma, acq_type='ei'):
        ...
        self.auxi_clf.fit(X, self.standard_bounds, batch_size=64, S=500)
        ...
        if len(X) > 1 and acq_type == 'ei':
            ...
            z_m = torch.empty(x_m.shape[0], dtype=torch.long).fill_(2)
            z = torch.cat([z_p, z_q, z_m], axis=0)
            z_onehot = torch.nn.functional.one_hot(z, num_classes=3).to(X.dtype)

            w = torch.ones(len(z), 1, dtype=X.dtype)
    ...
    def observe_and_suggest(self, X_obs, y_obs, X_pen=None, batch_size=1, gamma=1/3, *args, **kwargs):
        ...
        y_obs_thresholded = self.objective_thresholding(y_obs, ref_point=-self.problem.ref_point.numpy())

        # scalarise the objective values
        _, y_scalarized = self.scalariser.get_ranks(y_obs_thresholded, return_scalers=True)
