...
def optimize_acq_w_lbfgs(model, known_x, known_y, num_grad_steps=15_000, num_candidates=100, pre_sample_size=100_000, device='cpu', dtype=torch.float64,
    ...
    def opt_f(x):
        ...
        x_eval = torch.tensor(x).view(-1, len(dims_w_gradient_opt)).to(device=device, dtype=dtype).requires_grad_(True)
    ...
    if num_grad_steps > 0 and len(dims_w_gradient_opt) > 0:
        ...
        results[:, dims_w_gradient_opt] = torch.tensor(res.x).to(device=device, dtype=dtype).view(-1, len(dims_w_gradient_opt))
