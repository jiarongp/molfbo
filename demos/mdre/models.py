...
def mdre_loss(logits, targets, weight, class_weight):
    log_softmax_denom = (class_weight * logits.exp()).sum(dim=1, keepdim=True).log()
    loss = (targets * (class_weight * (- weight.log() - logits + log_softmax_denom))).sum(dim=1, keepdim=True)
    return loss.mean()
...
class BDRE:
    ...
    def fit(self, x, z, w, batch_size=64, S=100):
        optimizer = torch.optim.Adam(self.clf.parameters())
...
class MDRE:
    ...
    def fit(self, x, z, w, batch_size=64, S=500):
...
class TPE:
    ...
    def fit(self, X, y, z=None):
        ...
        self.kde_good.fit(X[z])
        self.kde_bad.fit(X[~z])
